...
QDialog *nGraphicsScene::createDialog(const QString &windowTitle) const
{
...
void nGraphicsScene::drawBackground(QPainter *painter, const QRectF &)
{
    if (painter->paintEngine()->type() != QPaintEngine::OpenGL &&
            painter->paintEngine()->type() != QPaintEngine::OpenGL2) {
        qWarning("nGraphicsScene: drawBackground needs a QGLWidget to be set as viewport on the graphics view");
        return;
    }
...
void nGraphicsScene::loadModel()
{
    loadModel(QFileDialog::getOpenFileName(0, tr("Choose model"), QString(), QLatin1String("*.obj")));
}

void nGraphicsScene::loadModel(const QString &filePath)
{
    if (filePath.isEmpty())
        return;
...
void nGraphicsScene::modelLoaded()
{
#ifndef QT_NO_CONCURRENT
...
void nGraphicsScene::setModel(Model *model)
{
    delete m_model;
    m_model = model;
...
void nGraphicsScene::enableWireframe(bool enabled)
{
    m_wireframeEnabled = enabled;
    update();
}

void nGraphicsScene::enableNormals(bool enabled)
{
    m_normalsEnabled = enabled;
    update();
}

void nGraphicsScene::setModelColor()
{
...
void nGraphicsScene::setBackgroundColor()
{
...
void nGraphicsScene::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
...
void nGraphicsScene::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void nGraphicsScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
...
void nGraphicsScene::wheelEvent(QGraphicsSceneWheelEvent *event)
{
